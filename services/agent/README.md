...
| 库存预警 | `get_live_room_snapshot` -> `get_inventory_status` |
| 订单查询 | `get_order_overview` |
| 售后辅助 | `get_order_overview` -> `get_after_sales_context` |
| 直播复盘 | `get_live_room_snapshot` -> `get_auction_history` -> `get_order_overview` -> `analyze_live_performance` |
...
Node 服务设置 `AGENT_BASE_URL=http://127.0.0.1:8100` 后，`/api/agent/chat` 会按登录角色传递可见的直播间、库存、订单和竞拍历史数据。主播可查看本直播间运营数据，买家只会向 Agent 提供自己的订单。
...
离线评测不依赖远程模型，覆盖规则问答、风险分析、主播话术、竞拍复盘、库存、订单、售后与直播复盘路径：
