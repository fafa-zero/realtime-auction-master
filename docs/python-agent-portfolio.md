...
我在一个 React + Node.js 实时竞拍系统旁路部署了 FastAPI Agent 服务：通过结构化上下文、固定工具计划、轻量 RAG 和短期会话记忆，为主播提供商品讲解、竞拍复盘、实时话术和异常出价解释；同时用离线评测、接口测试、WebSocket 契约测试和 CI 保证 Agent 不越权、不影响交易主流程。
...
| Agent 编排 | 意图路由、固定工具计划、结构化 tool results、模型失败兜底 |
| RAG | 基于竞拍规则和 AI 合规规范的轻量词法检索，并返回 citations |
| 记忆 | 按用户、直播间和 session 隔离的有界 TTL 会话记忆 |
| Agent 安全 | Agent 只读 Node 传入上下文，不能出价、支付或修改订单 |
...
2. 打开主播端，调用商品讲解或异常出价提示，展示 `source`、`toolsUsed` 和 `toolResults`。
