...
| 动态工具调用 | 原生手写 `tool_calls` 循环（`/v1/agent/dynamic`）与 LangGraph `StateGraph` 变体（`/v1/agent/graph`），共用同一套只读工具、策略、熔断和兜底，可在同一评测集上对比 |
...
- LangGraph 是可选依赖：生产默认走无框架的原生动态循环，未安装 LangGraph 时 `/v1/agent/graph` 自动降级到本地兜底；引入它主要用于框架对比与简历展示，而非核心依赖。
