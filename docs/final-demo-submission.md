...
> 说明：本文档用于最终提交页信息整理。带有 `待填写` 的内容需要在正式提交前补充真实信息。当前代码项目主要完成“实时竞拍”全栈 MVP，AI 能力尚未在代码中接入，建议提交前至少补齐一个 AI 功能点。
...
6. AI 辅助能力  
   建议补充：AI 生成商品直播讲解词、AI 生成竞拍复盘总结、AI 提示异常出价风险。
...
备注：当前本地目录未能读取到有效 Git 仓库信息，正式提交前建议上传到 GitHub 或 GitLab，并补充主分支、最后提交 hash 和提交时间。
...
当前代码仓库尚未接入真实大模型 API。项目目前主要完成实时竞拍全栈能力，包括前端页面、后端 API、Socket.IO 实时通信、竞拍状态机和模拟订单。

由于比赛主题是“AI 全栈挑战赛”，正式提交前建议至少补齐一个 AI 能力，避免项目被评委认为 AI 特征不足。

### 11.2 建议接入的 AI 能力

建议优先接入以下 3 个轻量 AI 能力：
